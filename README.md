...
## 使用方法
从 Release 中挑选合适版本下载后重命名为 claude-go（扩展名根据情况补充），然后添加到环境变量
以 Windows 为例：
可以创建并添加到 C:/apps/claude-go 目录下，
将 `C:/apps/claude-go`添加到系统环境变量路径
在终端中输入 claude-go 即可使用
初次使用时在输入栏中输入 /model 命令根据提示配置自己的 LLM API 及模型选择
![3](./assets/3.png)
